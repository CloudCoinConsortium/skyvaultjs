...
class SkyVaultJS {
  ...
  async apiDeleteRecord(params, callback = null) {
    ...
    if (!('guid' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_GUID, "GUID in missing");
    let guid = params['guid'];
    if (!this._validateGuid(guid)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_GUID, "Invalid GUID");
  ...
  async apiShowRecords(params, callback = null) {
    ...
    if ('start_ts' in params) {
      ts = params['start_ts'];
      if (!Number.isInteger(ts)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_TIMESTAMP, "Invalid Timestamp");
      ...
      this._parseMainPromise(response, 0, rv, (serverResponse, rIdx) => {
        ...
        if (serverResponse.status == "success") {
          ...
          for (let r = 0; r < data.length; r++) {
            let ldata = data[r];
            if (typeof ldata == 'undefined') continue;

            if (!('stripe' in ldata) || !('mirror' in ldata) || !('mirror2' in ldata)) {
              continue;
            }

            if (!('statement_id' in ldata)) continue;
  ...
  */
  ...
  async apiCreateCCForRegistration(params) {
    ...
    if (!('email' in params)) return this._getError("Email is not defined");
  ...
  async apiReceive(params, callback = null) {
    ...
    if (coinsToReceive.length > 0) {
      ...
      for (let i = 0; i < this._totalServers; i++) {
        ...
        for (let j = 0; j < coinsToReceive.length; j++) d.setUint32(52 + j * 3, coinsToReceive[j].sn << 8);
      ...
      for (let k in response.result) {
        for (let i = 0; i > 25; i++) response.result[k]['an'][i] = (0, _jsMd.default)(k.toString() + response.result[k]['sn'].toString() + coin.pan[i]); //delete(response.result[k]['message'])
  ...
  async _doTransfer(coin, to, tags, coinsToSend, callback, iteration) {
    ...
    for (let i = 0; i < this._totalServers; i++) {
      ...
      for (let j = 0; j < coinsToSend.length; j++) d.setUint32(52 + j * 3, coinsToSend[j].sn << 8);
  ...
  async apiFixTransferGeneric(coinsPerRaida, sync, callback) {
    ...
    if (typeof coinsPerRaida != "object") return this._getError("Failed to validate input args");
    let rqdata = [];
    ...
    for (let k in coinsPerRaida) {
      ...
      if (yes != 0 && no != 0) {
        ...
        for (let r = 0; r < raidas.length; r++) {
          ...
          if (rqdata[rIdx].sn.length >= this.options.maxCoinsPerIteraiton) continue;
  ...
  async apiGetFreeCoin(hwId) {
    ...
    if (response.status != 200) return this._getError("Failed to get free coin");
    let data = response.data;
    if (!('status' in data)) return this._getError("Failed to parse respense from FreeCoin Server");
    if (data.status != "success") return this._getError(data.message);
  ...
  async _realFix(round, raidaIdx, coins, callback = null) {
    ...
    for (let j = 0; j < 25; j++) {
      if ('tickets' in rcoins[coins[0].sn]) d.setUint32(33 + 3 * coins.length + 16 + j * 4, resultData['result'][coins[0].sn].tickets[i]);else {
    ...
    for (let i = 0; i < this._totalServers; i++) {
      ...
      for (let j = 0; j < amount; j++) {
        ...
        for (let x = 0; x < 16; x++) {
          ...
          if (pan) d.setUint8(33 + j * bodylength + (19 + x), parseInt(coin.pan[i].substr(x * 2, 2), 16));
  ...
  _wsConnect(url, data, i, timeout = 10000) {
    return new Promise(function (res, rej) {
      ...
      let socket = new _ws.default(url);
