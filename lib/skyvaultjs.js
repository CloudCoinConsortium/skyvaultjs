...
class SkyVaultJS {
  ...
  */
  ...
  async apiFixTransferGeneric(coin, bal = null, callback) {
    ...
    if (bal == null) bal = await this.apiShowBalance(coin, callback);
    let lower = [];
    let greater = [];
    let maxB = 0;
    let trueBal = 0;
    ...
    for (let k = 0; k < bal.balancesPerRaida.length; k++) {
      if (bal.balancesPerRaida[k] < trueBal && bal.balancesPerRaida[k] != null) lower.push[k];else if (bal.balancesPerRaida[k] > trueBal && bal.balancesPerRaida[k] != null) {
        greater.push[k];
      }
    }
    ...
    if (greater.length > 0) {
      for (let g in greater) {
        ...
        for (let sn in showcoin.coinsPerRaida) if (showcoin.coinsPerRaida[sn][g] = 'yes') sns.push(sn);
  ...
  */
  ...
  _gradeCoin(a, f, e) {
    if (a + f + e < this._activeServers.length) return this.__errorResult;
  ...
  _wsConnect(url, data, i, timeout = 10000) {
    ...
    if (this._activeServers.includes(i)) reject = false;
