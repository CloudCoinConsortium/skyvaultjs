    ...
    */
    ...
    for (let i = 0; i < this._totalServers; i++) {
      ...
      for (let j = 0; j < coins.length; j++) {
        if (coins[j].pownArray[i] == 'p') continue;
        ctfix.push(coins[j]);
      }

      if (ctfix.length != 0) {
        console.log("fixing coin", ctfix);
...
let details = [];
    for (let sn in showcoin.coinsPerRaida) {
      ...
      if (nocount > 0 && nocount < 10){
        console.log("adding ", sn,"to raida", l,"on id coin", coin.sn);
        details.push(await this._syncOwnersAddDelete(coin, [sn], l, 0));
      }
      if (yescount > 0 && yescount < 10){
        console.log("deleting ", sn,"to raida", g, "on id coin", coin.sn);
