...
var _promise = _interopRequireDefault(require("promise.allsettled"));
...
class SkyVaultJS {
  ...
  constructor(options) {
    ...
    if (this.options.sentryDSN) this.initSentry();
  } // Init Sentry
  ...
  initSentry() {
    Sentry.init({
      ...
      release: "skyvaultjs@" + _package.version,
      normalizeDepth: 10,
      sendDefaultPii: true,
  ...
  addBreadCrumb(msg, data = null) {
    if (!this.options.sentryDSN) return;
  ...
  async apiPown(params, callback = null) {
    ...
    let rqdata = this._formRequestData(params); // Launch Requests
  ...
  async apiDetect(params, callback = null) {
    ...
    let rqdata = this._formRequestData(params, false, 1); // Launch Requests
  ...
  async apiDeleteRecord(params, callback = null) {
    if (!('coin' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_COIN, "Coin in missing");
    ...
    let mainPromise = rqs.then(response => {
      ...
      if (!this._validResult(result)) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to delete statement. Too many error responses from RAIDA");
      return rv;
    });
    return mainPromise;
  ...
  async apiShowRecords(params, callback = null) {
    ...
    let mainPromise = rqs.then(response => {
          ...
          */
          ...
          if (dView.byteLength > 16) {
            ...
            while (unread) {
              ...
              let x = 0;
              let memobytes = []; //let endcheck = data.getUint16(nonmemo + x);

              while (x < 50) {
                //endcheck != 0) {
                memobytes.push(data.getUint8(nonmemo + x));
                x++; //endcheck = data.getUint16(nonmemo + x);
              }
      ...
      if (n > 20) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_RECORD_NOT_FOUND, "No Statements found");
      if (!this._validResult(result)) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to read statements. Too many error responses from RAIDA");

      for (let statement_id in statements) {
        let item = statements[statement_id]; //let odata = this._getDataFromObjectMemo(item.mparts)
        //if (odata == null) {
  ...
  async apiCreateRecord(params, callback = null) {
    ...
    if (!('coin' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_COIN, "Coin in missing");
    if (!('amount' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_AMOUNT, "Amount in missing");
    let coin = params['coin'];
    ...
    let mainPromise = rqs.then(response => {
      ...
      let result = this._gradeCoin(a, f, e); //      if (result == this.__counterfeitResult)
      //        return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_COIN, "The coin is counterfeit")


      if (!this._validResult(result)) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to create statement. Too many error responses from RAIDA");
      return rv;
    });
  ...
  async _getDefaultTicket(cc, callback) {
    ...
    if (response.status != "done") return null;
  ...
  async apiDeleteSkyWallet(params, callback = null) {
    ...
    if (!('coin' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_COIN, "Coin in missing");
    if (!('name' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_DNS_NAME, "DNS Name in missing");
    let coin = params['coin'];
    ...
    if (response.status != 200) return this._getErrorCode(SkyVaultJS.ERR_DNS_SERVER_INCORRECT_RESPONSE, "DNSService returned wrong code: " + response.status);
    let data = response.data;
    if (!('status' in data)) return this._getErrorCode(SkyVaultJS.ERR_DNS_SERVER_INCORRECT_RESPONSE, "DNSService returned wrong data. No status found");
  ...
  async apiViewreceipt(params, callback = null) {
    ...
    if (params.account < 1 || params.account > 16777216) return this._getError("Invalid Account");
    if (!params.tag.match(/^[a-fA-F0-9]{32}$/)) return this._getError("Invalid Tag UUID");
    let rqdata = [];
    ...
    let mainPromise = rqs.then(response => {
      for (let i = 0; i < response.length; i++) {
        if (typeof response[i].value == 'undefined') continue;
        if (typeof response[i].value.data != 'object') continue;
        response[i].value.data.status = "pass";
      }

      this._parseMainPromise(response, 0, rv, serverResponse => {
        if (typeof serverResponse != 'object') return;
      ...
      rv.sns = Object.keys(rv.sns).filter(item => {
        ...
        if (this._validResult(result)) return true;
        return false;
      });

      for (let sn of rv.sns) rv.total += this.getDenomination(sn);
  ...
  */
  ...
  async apiFixfracked(params, callback = null) {
    ...
    let coins = [];
    let superfixCoins = []; // Filter out fracked coins

    for (let k in params) {
      let coin = params[k];
      if (!('pownstring' in coin)) continue;
      if (!('result' in coin)) continue;
      if (coin.result != this.__frackedResult) continue;
      if (!this._validateCoin(coin)) continue;
    ...
    */
    ...
    for (let i = 0; i < this._totalServers; i++) {
      ...
      for (let j = 0; j < coins.length; j++) {
        if (coins[j].pownArray[i] != 'f') continue;
        ctfix.push(coins[j]);
      }
    ...
    for (let i = 0; i < coins.length; i++) {
      ...
      for (let j = 0; j < coins[i].pownArray.length; j++) {
        if (coins[i].pownArray[j] == 'p') a++;else if (coins[i].pownArray[j] == 'f') c++;else e++;
    ...
    for (let i = 0; i < superfixCoins.length; i++) {
      ...
      for (let j = 0; j < superfixCoins[i].pownArray.length; j++) {
        if (superfixCoins[i].pownArray[j] == 'p') a++;else if (superfixCoins[i].pownArray[j] == 'f') c++;else e++;
  ...
  async apiGetCCByCardData(params) {
    ...
    if (!('cardnumber' in params)) return this._getError("Card Number is not defined");
    if (!('cvv' in params)) return this._getError("CVV is not defined");
    if (!('username' in params)) return this._getError("Username is not defined");
    let cardNumber = params['cardnumber'];
    let cvv = params['cvv'];
    let username = params['username'];
    if (!this._validateCard(cardNumber, cvv)) return this._getError("Invalid Card");
    let sn = await this._resolveDNS(username);
    if (sn == null) return this._getError("Failed to resolve DNS");
    let part = cardNumber.substring(3, cardNumber.length - 1);
    let ans = [];

    for (let i = 0; i < 25; i++) {
      let seed = "" + i + sn + part + cvv;
      ans[i] = "" + _cryptoJs.default.MD5(seed);
    }
  ...
  async apiCreateCCForRegistration(params) {
    ...
    if (!('sn' in params)) return this._getError("SN is not defined");
    if (!('password' in params)) return this._getError("Password is not defined");
    let password = params['password']; //let email = params['email']

    if (password.length < this.options.minPasswordLength) return this._getError("Password length must be at least 16 characters");
    let sn = params['sn'];
    let finalStr = "";

    let tStr = "" + _cryptoJs.default.MD5(password);
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const p = '' + _cryptoJs.default.MD5(seed);

      const p0 = p.substring(0, 24);
      let component = '' + sn + '' + i;
      component = '' + _cryptoJs.default.MD5(component);
  ...
  async apiGetCCByUsernameAndPassword(params) {
    ...
    if (name == null) return this._getError("Failed to resolve DNS");
    let sn = name;
    let finalStr = "";
    ...
    for (let i = 0; i < this._totalServers; i++) {
      ...
      component = '' + _cryptoJs.default.MD5(component);
      const p1 = component.substring(0, 8);
      pans[i] = p0 + p1; //pans[i] = "" + CryptoJS.MD5(seed)
    }
  ...
  async apiLoginByUsernameAndPassword(params) {
    ...
    rv.details = detect;
    if (detect.details.authenticNotes > 0 || detect.details.frackedNotes > 0) rv.status = 'success';else rv.status = 'failure';
  }
  ...
  async extractStack(params) {
    ...
    let imgAx = _axios.default.create();
    ...
    if (isError) return this._getError("Failed to load image");
    if (response.status != 200) return this._getError("Server returned non-200 HTTP code: " + response.status);
    let arrayBufferData = response.data;
    let imgData = new Uint8Array(arrayBufferData);

    let idx = this._basePngChecks(imgData);

    if (typeof idx == 'string') return this._getError(idx);
    let fu8;
    ...
    while (true) {
      ...
      let signature = String.fromCharCode(fu8[i + 4]) + String.fromCharCode(fu8[i + 5]) + String.fromCharCode(fu8[i + 6]) + String.fromCharCode(fu8[i + 7]);
  ...
  async apiRestoreCard(params, callback = null) {
    if (!('username' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_DNS_NAME, "Username is not defined");
    let username = params['username'];
    let sn = await this._resolveDNS(username);
    if (sn == null) return this._getErrorCode(SkyVaultJS.ERR_DNS_RECORD_NOT_FOUND, "Failed to resolve SkyWallet");
    params.sn = sn;
    let rv = await this.apiCreateCCForRegistration(params);
    if (rv.status != "done") return rv;
    ...
    if (!this._validateCard(cardNumber, rv.cvv)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_CARD, "Invalid Card");
    ...
    if (res.code != SkyVaultJS.ERR_NO_ERROR) return res;
    rv = {
      'code': SkyVaultJS.ERR_NO_ERROR,
      'cardnumber': cardNumber,
      'cvv': rv.cvv,
  ...
  async apiGenerateCard(params, callback = null) {
    ...
    if (!('cardnumber' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_CARD_NUMBER, "Card Number is not defined");
    if (!('cvv' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_CVV, "CVV is not defined");
    if (!('username' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_DNS_NAME, "Username is not defined");
    let username = params['username'];
    if (!('expiration_date' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_EXPIRATION_DATE, "Expiration Date is not defined");
    let sn = await this._resolveDNS(username);
    if (sn == null) return this._getErrorCode(SkyVaultJS.ERR_DNS_RECORD_NOT_FOUND, "Failed to resolve DNS");
    let cardnumber = params['cardnumber'];
    let cvv = params['cvv'];
    if (!this._validateCard(cardnumber, cvv)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_CARD, "Invalid Card or CVV");
    let ed = params['expiration_date'];
    if (!/^\d{1,2}\/\d{2}/.test(ed)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_EXPIRATION_DATE, "Invalid Expiration Date");
    let cardData = await this.apiGetCCByCardData(params);
    if (cardData.status != 'done') return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_CARD, "Failed to Generate CloudCoin");
    let cc = cardData.cc;
    ...
    if (response.status != 200) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_INVALID_HTTP_RESPONSE, "Invalid Code " + response.status);
    if (response.headers['content-type'] != "image/png") return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_INVALID_HTTP_CONTENT_TYPE, "Downloaded template is not a PNG file");
    ...
    let ddata = await this.apiDrawCardData(data, username, cardnumber, cvv, ed, ip); // Embed Stack
    ...
    if ('status' in params && params.status == 'error') return this._getErrorCode(SkyVaultJS.ERR_FAILED_TO_EMBED_STACK, "Failed to embed stack");
    let rv = {
      'code': SkyVaultJS.ERR_NO_ERROR,
      'text': 'Card Generated',
      'data': bdata
  ...
  async apiDrawCardData(data, username, cardnumber, cvv, ed, ip) {
    ...
    try {
      ...
      if (!c) return this._getErrorCode(SkyVaultJS.ERR_NO_CANVAS_MODULE, "Canvas module not found");
    } catch (e) {
  ...
  async embedInImage(params) {
    ...
    let ccLength = data.length; // length + type + crc32 = 12 bytes
    ...
    myu8[6] = 0x44;
    myu8[7] = 0x63;
    let tBuffer = Buffer.from(data); // Data
  ...
  async apiReceive(params, callback = null) {
    ...
    if (coin == null) return this._getError("Failed to parse coin from params");
    /*
    ...
    */
    ...
    if ('code' in gcRqs && gcRqs.code == SkyVaultJS.ERR_COUNTERFEIT_COIN) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "The coin is counterfeit");
    ...
    */
    ...
    if (coinsToReceive.length > 0) {
      ...
      for (let i = 0; i < this._totalServers; i++) {
        if (seed != null) coin.pan[i] = (0, _jsMd.default)(i.toString() + seed);
      ...
      for (let k in response.result) {
        ...
        for (let i = 0; i < 25; i++) {
          let newpan = (0, _jsMd.default)(i.toString() + response.result[k]['sn'].toString() + coin.pan[i]);
      ...
      return response; //} else if (changeCoin === 0) {
  ...
  async apiBreakInBank(extraSn, idcc, callback) {
    this.addBreadCrumbEntry("apiBreakInBank", {
      'extraSn': extraSn,
      'idcc': idcc
    });
    let csns = [];
    ...
    let rv = await this._parseMainPromise(response, 0, {}, response => {
      if (response == "error" || response == "network") return;
  ...
  async apiPay(params, callback = null) {
    ...
    if (!('sender_name' in params)) return this._getError("Sender Name is required");
    let sender_address = params.sender_name;
    if (!('to' in params)) return this._getError("To is required");
    let from = sender_address;
    if ('from' in params) from = params.from;else params.from = from;
    let memo = "";
    if ('memo' in params) memo = params.memo;
    if (!('amount' in params)) return this._getError("Invalid params. Amount is not defined");
    let guid = "";
    ...
    if (!('guid' in params)) {
      ...
      guid = params.guid;
      if (!/^([A-Fa-f0-9]{32})$/.test(guid)) return this._getError("Invalid GUID format");
    }
  ...
  async apiBillPay(params, callback = null) {
    ...
    if (!('coin' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_COIN, "Coin is missing");
    let coin = params['coin'];
    ...
    if (v != null) {
      ...
      if (!('paydata' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_BILLPAY_MISSING_PAYDATA, "Paydata is missing");
      let id = this.genene;
      if (!('paydata' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_BILLPAY_MISSING_PAYDATA, "Paydata is missing");
      ...
      for (let i = 0; i < paydataArray.length; i++) {
        ...
        if (method != "TransferToSkywallet") return this._getErrorCode(SkyVaultJS.ERR_PARAM_BILLPAY_PAYDATA_INVALID_METHOD, "Invalid method. Line " + line);
        if (fformat != "stack") return this._getErrorCode(SkyVaultJS.ERR_PARAM_BILLPAY_PAYDATA_INVALID_FILE_FORMAT, "Only stack format supported. Line " + line);
        let amount = els[2].trim();
        if (!Number.isInteger(amount)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_BILLPAY_PAYDATA_INVALID_AMOUNT, "Incorrect Amount. Line " + line);
        ...
        if (state != "ready" && state != "skip") return this._getErrorCode(SkyVaultJS.ERR_PARAM_BILLPAY_PAYDATA_INVALID_STATUS, "Invalid status. Line " + line);
        if (to in paydata) return this._getErrorCode(SkyVaultJS.ERR_PARAM_BILLPAY_PAYDATA_DUPLICATED_VALUE, "Duplicated value for " + to + ". Line " + line);
        paydata[to] = {
          'amount': amount,
          'state': state,
          'memo': memo
  ...
  apiBillPayList(params) {
    if (!('guid' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_GUID, "GUID is required");
    let guid = params.guid;
    if (!this._validateGuid(guid)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_GUID, "Failed to validate GUID");

    let obj = this._getBillPayCachedObject(guid);

    if (obj == null) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_RECORD_NOT_FOUND, "BillPay List not found");
    let rv = {
      code: SkyVaultJS.ERR_NO_ERROR,
      amount: 0,
      amounttotal: 0,
    ...
    for (let to in obj) {
      let item = obj[to];
      if (item.state == "sent") rv.amount += item.amount;
  ...
  async apiTransfer(params, callback = null) {
    ...
    */
    ...
    let coinsToSend = rvalues.coins; //let changeCoin = rvalues.extra
  ...
  async apiFixTransferGeneric(coin, sync, callback) {
    ...
    if (typeof coin != "object") return this._getError("Failed to validate input args");
    ...
    for (let k = 0; k < this._totalServers; k++) {
      if (bal.balancesPerRaida[k] < bal.balance && bal.balancesPerRaida[k] != null) lower.push[k];else if (bal.balancesPerRaida[k] = bal.balance) {
        greater.push[k];
      }
    }

    if (lower.length <= 12) is_add = 1;else if (greater.length <= 12) is_add = 0;
    let sns = [];
    ...
    if (is_add = 1) {
      ...
      for (let l in lower) {
        ...
        if (missing1s > 0) {
          ...
          for (let sn in showcoin.coinsPerRaida) if (showcoin.coinsPerRaida[sn][l] = 'no') sns.push(sn);
        }
      ...
      for (let g in greater) {
        ...
        if (missing1s > 0) {
          ...
          for (let sn in showcoin.coinsPerRaida) if (showcoin.coinsPerRaida[sn][l] = 'yes') sns.push(sn);
        }
  ...
  async apiShowCoinsAsArray(coin, callback) {
    ...
    if ('code' in d && d.code == SkyVaultJS.ERR_COUNTERFEIT_COIN) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "The coin is counterfeit");
    let coins = d.coins;
    let a = [];
  ...
  async apiRecoverIDCoin(params, callback) {
    ...
    if (!('paycoin' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_COIN, "PayCoin in missing");
    let coin = params['paycoin'];
    if (!this._validateCoin(coin)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_COIN, "Failed to validate paycoin");
    if (!('skywallet_name' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_DNS_NAME, "Skywallet Name is missing");
    if (!('email' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_EMAIL, "Email is missing");
    ...
    let mainPromise = rqs.then(response => {
      ...
      if (!this._validResult(result)) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to send request. Too many error responses from RAIDA");
      return rv;
    });
  ...
  async apiHealthCheck(params, callback = null) {
    ...
    lrv = await this.apiShowCoins(coin, callback); // Check if the coins is counterfeit

    if ('code' in lrv && lrv.code == SkyVaultJS.ERR_COUNTERFEIT_COIN) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "The coin is counterfeit");
  ...
  async apiNFTExists(params, callback = null) {
    ...
    if (!('coins' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_COIN, "Coins are missing");
    let coins = params.coins;
    if (coins.length > this.options.maxCoinsPerIteraiton) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_COIN, "Too many coins in stack. Max number of coins is: " + this.options.maxCoinsPerIteraiton);

    for (let i = 0; i < coins.length; i++) {
      let coin = coins[i];
      if (!this._validateCoin(coin)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_COIN, "Failed to validate coins");
    }
    ...
    let mainPromise = rqs.then(response => {
      ...
      if (!this._validResult(result)) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to get NFT tokens. Too many error responses from RAIDA");
      let fresults = {};
  ...
  async apiNFTMultiInsert(params, callback = null) {
    ...
    if (!('coins' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_COIN, "Coins are missing");
    if (!('data' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_DATA, "Data is missing");
    let coins = params.coins;
    if (coins.length > this.options.maxCoinsPerIteraiton) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_COIN, "Too many coins in stack. Max number of coins is: " + this.options.maxCoinsPerIteraiton);
    ...
    if (detect.code != SkyVaultJS.ERR_NO_ERROR) return this._getErrorCode(SkyVaultJS.ERR_DETECT_FAILED, "Failed to do multi_detect");
    ...
    if (detect.frackedNotes > 0) {
      ...
      if (fresult.code != SkyVaultJS.ERR_NO_ERROR) return this._getErrorCode(SkyVaultJS.ERR_FAILED_TO_FIX, "Failed to fix coins");
  ...
  async apiNFTInsert(params, callback = null) {
    ...
    if (!('coin' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_COIN, "Coin is missing");
    if (!('data' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_DATA, "Data is missing");
    if (!('metadata' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_METADATA, "MetaData is missing");
    let metadata = params.metadata;
    if (!('filename' in metadata)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_MISSING_METADATA, "Filename is missing");
    let filename = metadata.filename;
    let proofmimetype = "image/jpeg";
    let mimetype = "application/octet-stream";
    if ('mimetype' in metadata) mimetype = metadata.mimetype;
    let coin = params['coin'];
    if (!this._validateCoin(coin)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_INVALID_COIN, "Failed to validate coin");
    let protocol = 0;
    if ('protocol' in params) protocol = params.protocol;
    if (protocol != 0 && protocol != 1) return this._getErrorCode(SkyVaultJS.ERR_PARAM_UNSUPPORTED_NFT_PROTOCOL, "Unsupported NFT Protocol");
    ...
    if (protocol == 1) {
      if (!('proofdata' in params)) return this._getErrorCode(SkyVaultJS.ERR_PARAM_NFT_MISSING_ID_PROOF, "ID Proof Picture is missing");
      if ('proofmimetype' in metadata) proofmimetype = metadata.proofmimetype;
      proofdata = params.proofdata;
      size += proofdata.length;
    }

    if (size > this.options.maxNFTSize) return this._getErrorCode(SkyVaultJS.ERR_PARAM_NFT_SIZE_IS_TOO_BIG, "The size of picture and ID proof is too big");
    let title = "";
    let description = "";
    ...
    let mainPromise = rqs.then(response => {
      ...
      if (!this._validResult(result)) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to create NFT token. Too many error responses from RAIDA");
      return rv;
    });
    this.addBreadCrumbReturn("apiNFTInsert", rv);
    return mainPromise;
  ...
  async apiNFTDelete(params, callback = null) {
    ...
    let mainPromise = rqs.then(response => {
      ...
      if (!this._validResult(result)) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to delete NFT token. Too many error responses from RAIDA");
      return rv;
    });
  ...
  async apiNFTRead(params, callback = null) {
    ...
    for (let i = 0; i < this._totalServers; i++) mparts[i] = null;
    ...
    let mainPromise = rqs.then(response => {
      ...
      if (!this._validResult(result)) return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to download NFT token. Too many error responses from RAIDA");
  ...
  async _resolveCloudFlareDNS(hostname, type = null) {
    let dnsAx = _axios.default.create({
      headers: {
  ...
  async _getCoinsWithContentBalance(coin, callback) {
    let rqdata = []; // Assemble input data for each Raida Server
  ...
  */
  ...
  async _getCoins(coin, callback = null) {
    //ByDenomination(coin, denomination, callback) {
    let rqdata = []; // Assemble input data for each Raida Server
    ...
    let rqs = this._launchRequests("show", rqdata, callback).then(response => {
      this._parseMainPromise(response, 0, rv, (response, rIdx) => {
        ...
        for (let x = 0; x < coins.byteLength - 1; x++) coinsView.setUint8(x, coinsplit.getUint8(x));
        ...
        for (let i = 0; i < amount; i++) {
          ...
          if (!(key in rv.coins)) {
            ...
            for (let j = 0; j < this._totalServers; j++) rv.coinsPerRaida[key][j] = "no";
          }
      ...
      if (result == this.__counterfeitResult) return this._getErrorCode(SkyVaultJS.ERR_COUNTERFEIT_COIN, "Counterfeit coins"); //if (!this._validResult(result))
      //  return this._getErrorCode(SkyVaultJS.ERR_RESPONSE_TOO_FEW_PASSED, "Failed to get coins. Too many error responses from RAIDA")
  ...
  async _realFix(round, raidaIdx, coins, callback = null) {
    ...
    for (let j = 0; j < 25; j++) {
      if ('tickets' in resultData.result[coins[0].sn]) d.setUint32(38 + 3 * coins.length + 16 + j * 4, resultData['result'][coins[0].sn].tickets[i]);else {
    ...
    for (let i = 0; i < this._totalServers; i++) {
      ...
      for (let j = 0; j < amount; j++) {
        ...
        for (let x = 0; x < 16; x++) {
          ...
          if (pan) d.setUint8(38 + j * bodylength + (19 + x), parseInt(coin.pan[i].substr(x * 2, 2), 16));
  ...
  _getGenericMainPromise(rqs, coins, gradeFunction = null) {
    ...
    let mainPromise = rqs.then(response => {
      ...
      this._parseMainPromise(response, 0, rv, (serverResponse, raidaIdx) => {
        ...
        let sr = new DataView(serverResponse); // The order in input and output data is the same

        for (let i = 0; i < coins.length; i++) {
          let sn = coins[i].sn; // serverResponse[i]
          ...
          if (status == 243) {
            if (sr.byteLength >= 20) ms = sr.getUint8(Math.floor(20 + i / 8));else if (sr.byteLength >= 16) {
          ...
          if (status == 250 || status == 241 || status == 243 && mixed == 1) {
            ...
            rcoins[sn].authentic++;
            rcoins[sn].pownstring += "p";
            if (!('tickets' in rcoins[sn])) rcoins[sn].tickets = [];
            if (sr.byteLength >= 16) rcoins[sn].tickets[raidaIdx] = sr.getUint32(3);
      ...
      Object.keys(rcoins).map(sn => {
        if (gradeFunction == null) {
          ...
          rcoins[sn].result = gradeFunction(rcoins[sn].authentic, rcoins[sn].counterfeit, rcoins[sn].errors, this);
        }
  ...
  _parseMainPromise(response, arrayLength, rv, callback) {
    for (let i = 0; i < response.length; i++) {
      ...
      serverResponse = response[i].value.data; //console.log('response from server ', dView.getUint8(0));
  ...
  _launchRequests(url, params = null, callback = null, servers = null, data = null) {
    ...
    return (0, _promise.default)(pms);
  }

  _initAxios() {
    this._axInstance = _axios.default.create({
      headers: {
  ...
  _validateCoin(coin) {
    ...
    if (typeof coin.an != 'object' || !Array.isArray(coin.an)) return false;

    if ('pan' in coin) {
      if (typeof coin.pan != 'object' || !Array.isArray(coin.pan)) return false;
    ...
    if (coin.an.some(elem => !elem.match(/^[a-fA-F0-9]{32}$/))) return false;
    return true;
  } // Validate GUID
  ...
  _assembleMessage(mparts) {
    ...
    for (let i = 0; i < this._totalServers; i++) {
      if (mparts[i] == null || typeof mparts[i] == 'undefined') continue;
      if (!mparts[i]['stripe'] || !mparts[i]['mirror1'] || !mparts[i]['mirror2']) continue;
      let cidx0 = i;
      let cidx1 = i + 3;
      let cidx2 = i + 6;
  ...
  _splitMessage(message) {
    ...
    if (pads > 0) for (let i = 0; i < this._totalServers - pads; i++) message += "-"; // Break the message

    let cs = message.split(''); // Init array
    ...
    for (let i = 0; i < cs.length; i++) {
      // Raida index
      let ridx = i % this._totalServers; // Chunk indexes
    ...
    for (let i = 0; i < this._totalServers; i++) {
      let cidx0 = i + 3;
      let cidx1 = i + 6;
      if (cidx0 >= this._totalServers) cidx0 -= this._totalServers;
      if (cidx1 >= this._totalServers) cidx1 -= this._totalServers;
  ...
  _get25B(sb, ss) {
    ...
    for (let i = 0; i < 4; i++) rsns[i] = sns[i];

    sns = this._getA(ss, 5);
    if (sns == null) return null;

    for (let i = 0; i < 5; i++) rsns[i + 4] = sns[i];
  ...
  _get100E(sb, ss, sss) {
    ...
    for (let i = 0; i < 3; i++) rsns[i] = sns[i];

    sns = this._getA(ss, 4);
    if (sns == null) return null;

    for (let i = 0; i < 4; i++) rsns[i + 3] = sns[i];

    sns = this._getA(sss, 5);
    if (sns == null) return null;

    for (let i = 0; i < 5; i++) rsns[i + 7] = sns[i];
  ...
  _get250E(sb, ss, sss, ssss) {
    ...
    for (let i = 0; i < 2; i++) rsns[i] = sns[i];
  ...
  _calcAmount(sns) {
    ...
    for (let i = 0; i < sns.length; i++) total += this.getDenomination(sns[i]);
  ...
  _getUint32(data, offset) {
    let a = data[offset] << 24 | data[offset + 1] << 16 | data[offset + 2] << 8 | data[offset + 3];
    return a >>> 0;
  } // network byte order
  ...
  _syncAdd(serverResponses, key, fcall) {
    ...
    for (let eid in hashData) {
      let quorum = Object.keys(hashData[eid]).length; // We believe that RAIDA servers that don't respond have our data
...
var _default = SkyVaultJS; // Es5 export
