...
class SkyVaultJS {
  ...
  async apiFind(params, callback = null) {
    ...
    for (let k in params.coins) {
      ...
      if (!('an' in coin)) continue;
      if (!('pan' in coin)) continue;
      coins.push(coin);
    }
  ...
  async apiSend(params, callback = null) {
    ...
    let ab, d; //for (let u = 0; u < packetscoins.length; u++) {

    let rqdata = [];
    let packetsize = 95;
    packetsize += 22; //header size

    packetsize += 19 * amountNotes; //if (u + 1 == packetscoins.length) packetsize += 2;
  ...
  async apiTransfer(params, callback = null) {
    ...
    let response = {};
    response = await this._doTransfer(coin, params.guid, to, tags, coinsToSend, callback); // Assemble input data for each Raida Server
  ...
  async apiNFTTestCreate(params, callback = null) {
    ...
    let response; // Assemble input data for each Raida Server
  ...
  async apiShowBalance(coin, callback) {
    ...
    let rqs = this._launchRequests("show_transfer_balance", rqdata, callback).then(response => {
      ...
      if (needFix) {
        ...
        coin.result = this.__frackedResult;
        this.apiFixfracked([coin]); //  return fpm;
      }
  ...
  _parseMainPromise(response, arrayLength, rv, callback) {
    ...
    for (let i = 0; i < response.length; i++) {
      ...
      console.log(serverResponse); //console.log('response from server ', dView.getUint8(0));
  ...
  _launchRequests(url, params = null, callback = null, servers = null, data = null) {
    ...
    let thiz = this; //   let tm = new Promise((res, rej) => {
  ...
  _generateServers() {
    if (this.options.wsprotocol == "wss") {
      ...
      this._raidaServers[23] = this.options.wsprotocol + "://213.59.119.96:8888";
  ...
  _calcAmount(sns) {
    ...
    for (let i = 0; i < sns.length; i++) total++;
