  ...
  */
  ...
  async apiReceive(params, callback = null) {
    ...
    if (coinsToReceive.length > 0) {
      ...
      for (let i = 0; i < this._totalServers; i++) {
        ...
        for (let j = 0; j < coinsToReceive.length; j++) d.setUint32(57 + j * 3, coinsToReceive[j].sn << 8);
  ...
  async _doTransfer(coin, to, tags, coinsToSend, callback, iteration) {
    ...
    for (let i = 0; i < this._totalServers; i++) {
      ...
      for (let j = 0; j < coinsToSend.length; j++) d.setUint32(57 + j * 3, coinsToSend[j].sn << 8);
  ...
  async _realFix(round, raidaIdx, coins, callback = null) {
    ...
    for (let j = 0; j < 25; j++) {
      if ('tickets' in rcoins[coins[0].sn]) d.setUint32(38 + 3 * coins.length + 16 + j * 4, resultData['result'][coins[0].sn].tickets[i]);else {
